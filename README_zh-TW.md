...
<a href="https://evolink.ai/gpt-5-6?utm_source=github&utm_medium=banner&utm_campaign=awesome-gpt-5.6-usecases&utm_content=readme_banner"><img src="https://pub-62cf7640cd0f4066b60933bd2e9b85ef.r2.dev/github-repo-media/awesome-gpt-5.6-usecases/images/zh-tw.png" alt="GPT-5.6 practical usecase repository banner" width="760"></a>
...
| 部分 | 案例 |
|---|---|
| [💻 編碼與構建](#coding-and-builds) | 32 案例 |
| [🤖 代理與工作流程](#agents-and-workflows) | 37 案例 |
| [🎨 創意與產品工作](#creative-and-product-work) | 28 案例 |
| [🧪 評估與限制](#evaluation-and-limits) | 50 案例 |
...
| 案例 | 它顯示了什麼 | 型 |
|---|---|---|
| [透過 iMessage 訓練個人模型](#case-1) | 使用 GPT-5.6 建立和運行本地訓練管道，從私人訊息歷史記錄中學習個人寫作風格。 | Demo |
| [運行為期一周的體素曼哈頓構建](#case-2) | 為長期運行的編碼代理提供體素曼哈頓構建，並讓它在多天內自主工作。 | Demo |
| [將口頭規範轉變為服務](#case-3) | 將口語自然語言規範轉變為端對端服務建構。 | Demo |
| [透過 Hermes 代理程式運行 GPT-5.6](#case-15) | 在 Hermes Agent 中使用 GPT-5.6，以 Nous Portal 作為模型存取層。 | Integration |
| [從桌面查看拉取請求](#case-20) | 使用統一的桌面應用程式進行內聯編輯、拉取請求審查、多儲存庫工作和更快的電腦使用。 | Integration |
| [建構互動式地球克隆](#case-22) | 使用長期運行的構建來組合 3D 地形、衛星圖像、天氣、搜尋和電影導航。 | Demo |
| [在最大模式下路由複雜的構建](#case-25) | 在 GPT-5.6 Sol 和 Fable 5 之間路由建置任務，同時將部署基礎架構保留在一個代理環境中。 | Integration |
| [选择 GPT-5.6 Copilot 等级](#case-26) | 將 Sol、Terra 或 Luna 與 GitHub Copilot 中的長時間運行推理、日常編碼或快速低成本任務相匹配。 | Integration |
| [建立訂閱產品頁面](#case-33) | 將創業想法轉變為有效的訂閱產品頁面，然後在發布前檢查定價和結帳狀態。 | Demo |
| [建立視覺提示工作室](#case-34) | 口述產品創意，然後讓客服人員實現畫布工具，將排列好的方框轉換為結構化圖像提示。 | Demo |
| [在AI SDK中啟用最大推理](#case-39) | 透過 AI SDK 選擇 GPT-5.6，並在應用程式程式碼中公開最大推理設定。 | Integration |
...
| [即時建立著陸頁](#case-102) | 當目標是從頭開始建立的完整登陸頁面時，在現場產品工程師課程中使用 GPT-5.6 Sol。 | Tutorial |
| [建構粒子地質演示](#case-110) | 使用 GPT-5.6 將科學解釋想法轉變為包含渲染物件、粒子過渡和交互物理的單頁畫布演示。 | Demo |
| [教授電影網站構建](#case-113) | 當目標是進階電影網站而不是靜態登陸頁面螢幕截圖時，請使用長格式的 GPT-5.6 Sol 教學。 | Tutorial |
| [建構 SQL 終端遊戲](#case-123) | 透過讓 GPT-5.6 Sol Ultra 將核心遊戲邏輯推入 SQLite，同時 Python 橋接運行時，建立不尋常的互動系統原型。 | Demo |
| [編寫代理程式可以讀取的程式碼](#case-124) | 優化編碼代理程式的程式碼結構和命名，使 GPT-5.6 Sol 在搜尋和檢索上花費更少的令牌。 | Tutorial |
| [組合前端設計插件](#case-125) | 當 Codex 前端建置需要更強的視覺效果時，將 GPT-5.6 與設計、動畫、產品和 Figma 外掛程式配對。 | Tutorial |
| [建構 Rust 機器人 Actor](#case-139) | 在使用編碼代理程式對機器人 Actor 系統進行原型設計時，將 Rust 即時核心與 Python 介面結合。 | Demo |
| [比較行動 UI 娛樂限制](#case-145) | 在信任 GPT-5.6 Sol 進行品牌敏感設計重建之前，跨模型運行相同的 Expo React Native UI 提示。 | Limit |
| [比較 React Native 應用程式構建](#case-146) | 在評估 GPT-5.6 Sol 與另一個前沿編碼模型時，比較真實裝置上已完成的行動應用程式。 | Evaluation |
...
| 案例 | 它顯示了什麼 | 型 |
|---|---|---|
| [建構個人業務作業系統](#case-4) | 從一項重複性任務開始，授予 Codex 對相關工具的存取權限，並在工作可靠運行後擴展工作流程。 | Tutorial |
| [新增自主批評通行證](#case-5) | 在審查 GPT-5.6 自己的工作時，使用第二個代理作為批評通行證。 | Demo |
| [將目標轉換為已完成的工作](#case-11) | 使用 ChatGPT Work 跨應用程式和檔案執行操作，在專案上停留數小時，然後返回已完成的工作。 | Demo |
| [西蘭花農場的自動化工作](#case-12) | 描述現場的操作問題，然後使用 GPT-5.6 圍繞該問題建立追蹤和自動化工作流程。 | Demo |
| [對較小的模型進行後期訓練](#case-16) | 為 GPT-5.6 Sol 提供完整的研究提示，並用它來設計 GPT-5.6 Luna 的後訓練流程。 | Demo |
| [執行多日瀏覽器和編碼任務](#case-19) | 使用一個代理程式進行小型編輯、瀏覽器操作和多日構建，同時保留對後續操作的批准。 | Evaluation |
| [在 Microsoft Foundry 部署代理](#case-31) | 將 GPT-5.6 與 Microsoft Foundry 中的託管代理程式結合使用，以實現託管企業代理程式工作流程。 | Integration |
| [在 Microsoft 365 中使用 GPT-5.6](#case-32) | 在 Word、Excel、PowerPoint、Chat 和 Copilot Cowork 知識工作任務中套用首選模型。 | Integration |
...
| [跨編碼代理路由工作](#case-98) | 保留 GPT-5.6 Sol 作為規劃線程，同時將瀏覽器、架構、長時間運行和快速更改工作路由到單獨的代理。 | Integration |
| [Dogfood 是一種承諾工具](#case-105) | 使用 GPT-5.6 Sol 代理建立個人工作流程應用程序，然後在投資更多代幣之前使用未完成的版本。 | Demo |
| [將 Kimi 配置為 Codex 子代理](#case-107) | 讓 GPT-5.6 Sol 負責 Codex 中的邏輯，同時將前端執行委託給 Kimi K3 OpenCode 子代理程式。 | Tutorial |
| [將寓言計畫轉為 GPT 編碼](#case-109) | 使用 Fable 進行規劃和判斷，然後將編碼任務發送到 GPT-5.6，而更便宜的或特定於編寫的子代理處理日常工作。 | Integration |
| [建置 ChatLLM 模型路由器](#case-111) | 建立一個自訂路由器，將簡單編碼、硬編碼和設計提示指派給不同的模型，包括 GPT-5.6 Sol。 | Integration |
| [啟動 Herdr Pi 代理程式工作流程](#case-115) | 將重複的 Claude Code 路由指令移至技能加路由參考中，以便 GPT-5.6 pi 代理程式在重點窗格中啟動。 | Integration |
| [使用 GPT 建構循環寓言規劃](#case-116) | 使用Fable作为规划者和审阅者，而GPT-5.6则反复实现和修复存储库更改。 | Integration |
| [Cap Codex 子代理程式遞歸](#case-119) | 透過故意路由模型層、限制子代理深度和添加明確停止檢查點來減少 GPT-5.6 限制消耗。 | Limit |
| [減少代理積分使用](#case-126) | 透過結合提示快取遙測、GPT-5.6 Terra 基礎模型和更嚴格的工具結果上下文，降低代理營運成本。 | Integration |
| [委託本地碼讀取](#case-128) | 將 GPT-5.6 作為推理線索，而本地 Qwen 代理並行讀取代碼並機械地驗證引用。 | Integration |
| [調整 OpenCode 令牌控制](#case-132) | 只有當您了解這些控制項所消除的工作流程權衡時，才能限制代理輸出和子代理程式產生。 | Limit |
| [將 GPT 版本與 Fable 評論配對](#case-134) | 分割編碼工作流程，以便 GPT-5.6 處理實作迭代，而 Claude Fable 保留上下文以供審核和回饋。 | Demo |
| [將 Codex 對應到宏硬體](#case-136) | 當鍵盤工作流程需要更快的代理命令時，將 Codex 操作對應到專用硬體控制項上。 | Integration |
| [以語音核准客服人員操作](#case-138) | 新增語音審核橋，以便助理可以在繼續任務之前確認有風險的代理操作。 | Integration |
| [透過 Hermes 加入 Slack 代理](#case-141) | 當入職需要可重複的管理工作時，請使用螢幕操作 GPT-5.6 Terra 代理程式來設定新的 Slack 代理程式。 | Integration |
| [自訂代理程式中的路由模型](#case-147) | 建立一個自訂編碼代理，將 GPT-5.6 Sol 路由到後端任務，而其他模型則發揮其優勢。 | Integration |
...
| 案例 | 它顯示了什麼 | 型 |
|---|---|---|
| [透過一條指令編輯炒作視頻](#case-6) | 將 MP4 放入工作區並請求使用自然語言進行簡潔的宣傳剪輯。 | Tutorial |
| [透過 MCP 製作品牌意識廣告](#case-7) | 將 GPT-5.6 連接到廣告製作 MCP，以便代理商可以在一個工作流程中使用品牌背景、視訊工具和可重複使用的技能。 | Integration |
| [擴展 Figma Make 中的現有設計](#case-8) | 根據現有設計進行建置時，在 Figma Make 中使用 GPT-5.6，然後比較輸出強度和令牌效率。 | Integration |
| [检查和优化渲染设计](#case-27) | 讓代理人檢查其渲染的輸出，發現視覺或功能缺陷，並在移交前進行修改。 | Demo |
| [创建交互式视觉解释](#case-29) | 將資料和概念轉化為圖表、演練、3D 模型、模擬、迷你應用程式和可分享網站。 | Demo |
| [生成演示文稿草稿](#case-30) | 使用 GPT-5.6 建立初始可編輯演示文稿，然後在交付前檢查結構和視覺品質。 | Demo |
| [整理詳細的圖片說明](#case-35) | 使用 GPT-5.6 建立長影像產生要求，然後將其傳送到未更改的影像引擎。 | Demo |
| [比較遊戲藝術中的燈光細節](#case-37) | 評估視覺細節時並排比較低角度照明輸出。 | Evaluation |
| [建立本機可編輯的 Office 文件](#case-38) | 將投影片、工作表和文件產生為可編輯工件，而非平面影像。 | Demo |
| [發布可過濾的城市指南](#case-40) | 按街區、心情和價格組織推薦，然後將結果發佈為網站。 | Demo |
...
| [比較 Ink-Wash 動作提示](#case-112) | 當風格、時間和變換約束很重要時，在同一電影運動提示上對照另一個模型評估 GPT-5.6 Sol。 | Benchmark |
| [基準個人網站重新設計](#case-114) | 在同一網站上比較模型－當身分適合、可訪問性、效能和複製品質都很重要時，會提示重新設計。 | Benchmark |
| [建構 Three.js 水著色器](#case-122) | 使用 GPT-5.6 Sol 進行專門的 Three.js 著色器工作，然後將模型產生的場景資源整合到一個瀏覽器演示中。 | Demo |
| [建立 3D 滾動網站](#case-127) | 使用 GPT-5.6 Sol 製作精美的 3D 滾動網站原型，然後在重複使用之前檢查生成的動作和佈局。 | Demo |
| [將參考表變成檢視器](#case-137) | 透過 Blender MCP 和 Codex 傳送參考表，以產生 3D 資源和可導航的 Web 檢視器。 | Demo |
| [壓力測試3D網站質量](#case-143) | GPT-5.6 Sol快速建立個人3D網站基準時，分別處理速度和輸出品質。 | Limit |
...
| 案例 | 它顯示了什麼 | 型 |
|---|---|---|
| [比較智慧、編碼、成本](#case-9) | 使用第三方基準測試結果，根據智慧、編碼效能和每項任務的成本來選擇 Sol、Terra 或 Luna。 | Benchmark |
| [根據成本測試物理質量](#case-10) | 在選擇 Sol Ultra 進行基於瀏覽器的模擬之前，請分別對視覺效果和物理正確性進行基準測試。 | Evaluation |
| [比較每美元的編碼性能](#case-13) | 將基準分數、任務成本和令牌使用一起比較，而不是單獨根據分數對編碼模型進行排名。 | Benchmark |
| [使用 ARC-AGI-3 測試小說推理](#case-14) | 使用 ARC-AGI-3 測試模型在不熟悉的互動式任務中的定位能力。 | Benchmark |
| [比較 DeepSWE 上的層級](#case-17) | 在選擇等級之前，先在相同軟體工程排行榜上比較 Sol、Terra 和 Luna。 | Benchmark |
| [審核一個月的代理構建](#case-18) | 在從大型模型試驗中得出結論之前，先審查代理人的整體活動和已完成的專案。 | Evaluation |
| [衡量代理績效與成本](#case-21) | 在相同的推理設定下比較代理基準分數和估計成本。 | Benchmark |
| [基準編碼效率](#case-23) | 評估編碼分數以及輸出標記、經過的時間和任務成本。 | Benchmark |
| [按任务形状选择模型](#case-24) | 使用 GPT-5.6 進行廣泛的知識工作循環和實施，同時為最困難的架構工作保留其他模型。 | Evaluation |
| [通用网络越狱帐户](#case-28) | 將網路安全防護措施視為部署約束，因為在測試中仍然可以進行長形式的代理攻擊。 | Limit |
...
| [追蹤降低後的 GPT-5.6 思考預算](#case-59) | 在假設更快的 GPT-5.6 Sol 執行仍保留同樣推理深度前，先比較可見的思考預算設定。 | Limit |
| [依任務選擇 GPT-5.6 層級](#case-61) | 將日常寫程式、全倉庫修改與最終審查分配到不同 GPT-5.6 層級，而不是預設使用 Ultra。 | Limit |
...
| [Benchmark WebDev Arena 排名](#case-79) | 選擇 GPT-5.6 Sol 進行網路開發工作時，使用人類投票前端排名作為一個訊號。 | Benchmark |
...
| [基準程序港口城鎮](#case-99) | 使用單一檔案程式模擬作為跨 GPT-5.6、Fable、Kimi 和 Inkling 的可重複創意編碼基準。 | Benchmark |
| [評估彈珠遊戲物理](#case-100) | 比較產生的遊戲簡報的控制對齊、重力感、完成時間和限制點擊次數，而不是只判斷螢幕截圖。 | Benchmark |
| [追蹤基準飽和度](#case-101) | 當 GPT-5.6 Sol Pro 飽和了大部分剩餘問題時，停用或重新設計基準。 | Benchmark |
| [基準個人基因組代理](#case-103) | 在信任任何單一模型配置之前，透過通過率和重複嘗試來評估基因組代理。 | Benchmark |
| [設定代理退出條件](#case-104) | 在要求 GPT-5.6 Sol Ultra 進行深入計劃審查之前，限制審查輪次並定義停止標準。 | Limit |
| [比較 Cyber Benchmark Windows](#case-106) | 在將一項 GPT-5.6 Sol 分數視為一般能力證據之前，請先按任務系列和時間視窗比較網路基準測試結果。 | Benchmark |
| [Benchmark React 專案修復](#case-108) | 使用 React 專案修復基準測試渲染、useEffect 用法、可存取性和可維護性，而不是依賴通用編碼排名。 | Benchmark |
| [基準國際太空站數位孿生](#case-117) | 使用詳細的 3D 數位孿生任務來測試重建、照明、控制、標籤、遊覽和細化行為。 | Benchmark |
| [衡量德語簡化推理](#case-118) | 當您需要不太可能被廣泛排行榜優化的推理訊號時，請執行 AlmanBench 等利基基準測試。 | Benchmark |
| [管理 GPT-5.6 iOS 會話](#case-120) | 每 30-60 分鐘檢查一次並在架構漂移複合之前進行重定向，保持 GPT-5.6 應用程式建構的協作性。 | Limit |
| [比較 Coding-Agent 修復線束](#case-121) | 透過成功率、嘗試次數、每次修復成本和掛機時間來評估保留修復套件上的編碼代理，而不僅僅是競技場排名。 | Benchmark |
| [比較蒸餾定律答案](#case-129) | 使用固定的政策敏感問題來比較 GPT-5.6 和同儕模型如何建構不確定性和風險。 | Evaluation |
| [在程式碼庫上執行 OpenBench](#case-130) | 在路由編碼代理程式工作之前，使用 OpenBench 在您自己的程式碼庫上比較模型和工具選擇。 | Benchmark |
| [比較真實付費編碼的工作](#case-131) | 跨模型執行相同的付費編碼作業，以區分品質、錯誤查找和成本，而不是依賴通用分數。 | Evaluation |
| [基準海岸線波浪代理](#case-133) | 使用海岸線波浪模擬來測試模型是否可以在長時間迭代視覺建構中應用研究概念。 | Evaluation |
| [透過向量搜尋對分類法進行分類](#case-135) | 當直接邊界呼叫對於大型分類法來說成本太高時，在分類之前使用向量搜尋來篩選標籤。 | Evaluation |
| [開放 EnigmaEval 推理基準](#case-140) | 在較長的解謎推理任務而不是較短的基準問題上比較 GPT-5.6 Sol 時，請使用 EnigmaEval。 | Benchmark |
| [測量 AutoCAD 計算機使用情況](#case-142) | 當電腦使用準確性比通用編碼分數更重要時，基準 GPT-5.6 Sol 可以執行精確的 AutoCAD 任務。 | Benchmark |
| [比較 VulcanBench 工程任務](#case-144) | 使用儲存庫層級隱藏測試基準來檢查 GPT-5.6 Sol 在實際工程變更方面是否保持競爭力。 | Benchmark |

## 用例

<a id="case-1"></a>
### Case 1: [透過 iMessage 訓練個人模型](https://x.com/skirano/status/2075283308276220011) (by [@skirano](https://x.com/skirano))

**使用 GPT-5.6 建立和運行本地訓練管道，從私人訊息歷史記錄中學習個人寫作風格。**

創建者報告說，一個提示生成了完整的訓練管道，根據 iMessage 歷史記錄在 Mac 上本地訓練了一個模型，並以創建者的風格生成了回复。
...
<a id="case-2"></a>
### Case 2: [運行為期一周的體素曼哈頓構建](https://x.com/mattshumer_/status/2075268746315268138) (by [@mattshumer_](https://x.com/mattshumer_))

**為長期運行的編碼代理提供體素曼哈頓構建，並讓它在多天內自主工作。**

創作者表示，GPT-5.6 Sol 在持續近一週的單次自主運作中產生了詳細的曼哈頓體素。
...
<a id="case-3"></a>
### Case 3: [將口頭規範轉變為服務](https://x.com/OpenAIDevs/status/2075279941906936035) (by [@OpenAIDevs](https://x.com/OpenAIDevs))

**將口語自然語言規範轉變為端對端服務建構。**

OpenAI Developers 重點介紹了 Ramp 工程師 Sam Kronick 使用 GPT-5.6 根據自然語言規範建立整個服務。
...
<a id="case-4"></a>
### Case 4: [建構個人業務作業系統](https://x.com/gregisenberg/status/2075278451116818795) (by [@gregisenberg](https://x.com/gregisenberg))

**從一項重複性任務開始，授予 Codex 對相關工具的存取權限，並在工作可靠運行後擴展工作流程。**

49 分鐘的大師班介紹了帶有草稿回复的收件匣卡、統一的 Slack 和會議源、代理電子郵件地址、學習的可重複技能以及長達 20 小時的目標。
...
<a id="case-5"></a>
### Case 5: [新增自主批評通行證](https://x.com/AdamHoltererer/status/2075338169000620322) (by [@AdamHoltererer](https://x.com/AdamHoltererer))

**在審查 GPT-5.6 自己的工作時，使用第二個代理作為批評通行證。**

創建者報告稱，GPT-5.6 自主啟動了另一個代理來批評其工作，即使沒有技能或指令明確要求該行為。
...
<a id="case-6"></a>
### Case 6: [透過一條指令編輯炒作視頻](https://x.com/clairevo/status/2075279502331248746) (by [@clairevo](https://x.com/clairevo))

**將 MP4 放入工作區並請求使用自然語言進行簡潔的宣傳剪輯。**

創作者示範了從 MP4 到 60 秒宣傳影片的工作流程，並連結了完整的 YouTube 教學。
...
<a id="case-7"></a>
### Case 7: [透過 MCP 製作品牌意識廣告](https://x.com/Creatify_AI/status/2075281407887409569) (by [@Creatify_AI](https://x.com/Creatify_AI))

**將 GPT-5.6 連接到廣告製作 MCP，以便代理商可以在一個工作流程中使用品牌背景、視訊工具和可重複使用的技能。**

Creatify 表示，其 MCP 可讓代理商在幾分鐘內從想法轉變為完成的廣告，並且可以存取完整的廣告堆疊。
...
<a id="case-8"></a>
### Case 8: [擴展 Figma Make 中的現有設計](https://x.com/figma/status/2075272590822625405) (by [@figma](https://x.com/figma))

**根據現有設計進行建置時，在 Figma Make 中使用 GPT-5.6，然後比較輸出強度和令牌效率。**

Figma 報告了早期測試，現有設計具有更強的輸出並提高了代幣效率。
...
<a id="case-9"></a>
### Case 9: [比較智慧、編碼、成本](https://x.com/ArtificialAnlys/status/2075268970492657905) (by [@ArtificialAnlys](https://x.com/ArtificialAnlys))

**使用第三方基準測試結果，根據智慧、編碼效能和每項任務的成本來選擇 Sol、Terra 或 Luna。**

Artificial Analysis 報告稱，Sol 的智慧指數為 59，編碼代理指數為 80； Terra 和 Luna 的智力得分為 55 分和 51 分，每項任務的成本較低。這些是來自來源的第三方基準聲明。
...
<a id="case-10"></a>
### Case 10: [根據成本測試物理質量](https://x.com/atomic_chat_hq/status/2075323372574068947) (by [@atomic_chat_hq](https://x.com/atomic_chat_hq))

**在選擇 Sol Ultra 進行基於瀏覽器的模擬之前，請分別對視覺效果和物理正確性進行基準測試。**

Atomic Chat 比較了三個 HTML5 畫布物理場景上的四個模型。據報道，Sol Ultra 使用了 32,900 個代幣，價格為 0.33 美元，而 GPT-5.5 使用了 12,400 個代幣，價格為 0.11 美元，儘管視覺細節更多，但物理性能較弱。
...
<a id="case-11"></a>
### Case 11: [將目標轉換為已完成的工作](https://x.com/OpenAI/status/2075274271845404744) (by [@OpenAI](https://x.com/OpenAI))

**使用 ChatGPT Work 跨應用程式和檔案執行操作，在專案上停留數小時，然後返回已完成的工作。**

OpenAI 引入了 ChatGPT Work 作為 GPT-5.6 支援的代理，它可以跨應用程式和檔案收集上下文，並可以持續專案幾個小時，直到產生最終結果。
...
<a id="case-12"></a>
### Case 12: [西蘭花農場的自動化工作](https://x.com/OpenAI/status/2075310019185389913) (by [@OpenAI](https://x.com/OpenAI))

**描述現場的操作問題，然後使用 GPT-5.6 圍繞該問題建立追蹤和自動化工作流程。**

在發布影片中，農民 Hiroki Tomiyasu 要求建立一個追蹤農場工作的系統。它顯示，儘管農民不是工程師，但 Codex 仍被用來實現溫室通風過程的自動化。
...
<a id="case-13"></a>
### Case 13: [比較每美元的編碼性能](https://x.com/bridgemindai/status/2075271377397981641) (by [@bridgemindai](https://x.com/bridgemindai))

**將基準分數、任務成本和令牌使用一起比較，而不是單獨根據分數對編碼模型進行排名。**

BridgeMind 報告稱，Fable 5 Max 的 CursorBench 結果為 70.5%，價格為 17.32 美元，有 103,525 個代幣，而 GPT-5.6 Sol Max 的結果為 67.2%，價格為 5.22 美元，有 28220 個代幣。
...
<a id="case-14"></a>
### Case 14: [使用 ARC-AGI-3 測試小說推理](https://x.com/arcprize/status/2075270869992264003) (by [@arcprize](https://x.com/arcprize))

**使用 ARC-AGI-3 測試模型在不熟悉的互動式任務中的定位能力。**

ARC Priest 報告了 GPT-5.6 Sol 的經過驗證的 7.8% 結果，並將其描述為第一個擊敗 ARC-AGI-3 遊戲的前沿模型。
...
<a id="case-15"></a>
### Case 15: [透過 Hermes 代理程式運行 GPT-5.6](https://x.com/NousResearch/status/2075270903458373640) (by [@NousResearch](https://x.com/NousResearch))

**在 Hermes Agent 中使用 GPT-5.6，以 Nous Portal 作為模型存取層。**

Nous Research 報告 Hermes Agent 中的 GPT-5.6 支援以及透過 Nous Portal 的可用性，建立了具體的代理-線束整合路線。
...
<a id="case-16"></a>
### Case 16: [對較小的模型進行後期訓練](https://x.com/sharifshameem/status/2075279959996928307) (by [@sharifshameem](https://x.com/sharifshameem))

**為 GPT-5.6 Sol 提供完整的研究提示，並用它來設計 GPT-5.6 Luna 的後訓練流程。**

Sharif Shameem 發布了用於 GPT-5.6 Sol 研究和執行 GPT-5.6 Luna 後訓練的完整提示。
...
<a id="case-17"></a>
### Case 17: [比較 DeepSWE 上的層級](https://x.com/datacurve/status/2075336589866316091) (by [@datacurve](https://x.com/datacurve))

**在選擇等級之前，先在相同軟體工程排行榜上比較 Sol、Terra 和 Luna。**

DataCurve 報告稱，GPT-5.6 在 DeepSWE 排行榜上名列前茅，得分為 73%，並發布了所有三個 GPT-5.6 級別的結果。
...
<a id="case-18"></a>
### Case 18: [審核一個月的代理構建](https://x.com/theo/status/2075439348267577790) (by [@theo](https://x.com/theo))

**在從大型模型試驗中得出結論之前，先審查代理人的整體活動和已完成的專案。**

Theo Browne 回顧了一個月的 GPT-5.6 Sol 使用情況。該影片展示了涵蓋 67 個項目和數十萬個提示的活動審核，然後介紹了代理實際構建的內容。
...
<a id="case-19"></a>
### Case 19: [執行多日瀏覽器和編碼任務](https://x.com/MatthewBerman/status/2075267049266487689) (by [@MatthewBerman](https://x.com/MatthewBerman))

**使用一個代理程式進行小型編輯、瀏覽器操作和多日構建，同時保留對後續操作的批准。**

Matthew Berman 報告了多天的遊戲和 Excel 構建、透過瀏覽器控制調整 Supabase 大小以及在相應保存之前暫停的 Google Workspace 遷移。他也記錄了剩餘的監督和設計限制。
...
<a id="case-20"></a>
### Case 20: [從桌面查看拉取請求](https://x.com/CodexReleases/status/2075265220054782386) (by [@CodexReleases](https://x.com/CodexReleases))

**使用統一的桌面應用程式進行內聯編輯、拉取請求審查、多儲存庫工作和更快的電腦使用。**

Codex Releases 記錄了桌面更新：內聯 Markdown 和程式碼編輯、側邊欄中的 GitHub 拉取請求審查、多儲存庫專案支援以及更快的 GPT-5.6 電腦使用。
...
<a id="case-21"></a>
### Case 21: [衡量代理績效與成本](https://x.com/OpenAI/status/2075271423992680532) (by [@OpenAI](https://x.com/OpenAI))

**在相同的推理設定下比較代理基準分數和估計成本。**

OpenAI 報告稱，GPT-5.6 Sol 在 Agents' Last Exam 中的得分為 53.6，比 Claude Fable 5 自適應高出 13.1 分，並表示中等推理比 Fable 5 領先 11.4 分，而成本約為估計成本的四分之一。
...
<a id="case-22"></a>
### Case 22: [建構互動式地球克隆](https://x.com/pankajkumar_dev/status/2075292795557040391) (by [@pankajkumar_dev](https://x.com/pankajkumar_dev))

**使用長期運行的構建來組合 3D 地形、衛星圖像、天氣、搜尋和電影導航。**

Pankaj Kumar 報告使用 GPT-5.6 Sol Ultra 和超過 3000 萬個代幣構建了一個 Google 地球克隆版，其中包含地形、圖像、地標、天氣、晝夜循環、遊覽和全球搜索。
...
<a id="case-23"></a>
### Case 23: [基準編碼效率](https://x.com/OpenAI/status/2075271425548795909) (by [@OpenAI](https://x.com/OpenAI))

**評估編碼分數以及輸出標記、經過的時間和任務成本。**

OpenAI 報告稱，GPT-5.6 Sol 的人工分析編碼代理指數為 80.0，比《寓言 5》高出 2.8 個點，同時使用了不到一半的輸出代幣和時間，成本也減少了約三分之一。
...
**使用 GPT-5.6 進行廣泛的知識工作循環和實施，同時為最困難的架構工作保留其他模型。**

Dan Shipper 的零日審核涵蓋編碼、寫作、設計、電子郵件、會議決策、候選人搜尋、市場掃描和膳食記錄。它將 GPT-5.6 評為快速且實用，但指出其高端編碼和設計性能較弱。
...
<a id="case-25"></a>
### Case 25: [在最大模式下路由複雜的構建](https://x.com/bindureddy/status/2075295728373833801) (by [@bindureddy](https://x.com/bindureddy))

**在 GPT-5.6 Sol 和 Fable 5 之間路由建置任務，同時將部署基礎架構保留在一個代理環境中。**

Bindu Reddy 將 Abacus AI Max 模式描述為 SaaS、自訂工作流程、行動應用程式和包含後端基礎設施的始終線上服務的兩種模型之間的路由。
...
**將 Sol、Terra 或 Luna 與 GitHub Copilot 中的長時間運行推理、日常編碼或快速低成本任務相匹配。**

GitHub 將 Sol 描述為大型程式碼庫和長時間運行的代理程式工作的最高選項，Terra 為平衡預設值，Luna 為輕量級低成本選擇。
...
**讓代理人檢查其渲染的輸出，發現視覺或功能缺陷，並在移交前進行修改。**

OpenAI 表示，GPT-5.6 將更強的設計判斷與電腦使用相結合，因此它可以檢查渲染結果，而不僅僅是產生程式碼或內容。
...
**將網路安全防護措施視為部署約束，因為在測試中仍然可以進行長形式的代理攻擊。**

英國人工智慧安全研究所報告了每輪測試中的通用越獄情況，包括長形式的漏洞發現和漏洞利用開發任務。
...
**將資料和概念轉化為圖表、演練、3D 模型、模擬、迷你應用程式和可分享網站。**

Andrew Ambrosino 描述了 GPT-5.6 產生視覺化和內嵌互動式工具，包括可操縱的模擬和可作為網站發布的收件匣公用程式。
...
**使用 GPT-5.6 建立初始可編輯演示文稿，然後在交付前檢查結構和視覺品質。**

創建者測試了直接 PPTX 生成並報告了輸出品質的改進，其中四個來源圖像顯示了生成的簡報頁面。
...
<a id="case-31"></a>
### Case 31: [在 Microsoft Foundry 部署代理](https://x.com/Azure/status/2075275078237852100) (by [@Azure](https://x.com/Azure))

**將 GPT-5.6 與 Microsoft Foundry 中的託管代理程式結合使用，以實現託管企業代理程式工作流程。**

Microsoft Azure 宣布 GPT-5.6 全面上市，同時 Foundry Agent Service 中的託管代理也全面上市。
...
**在 Word、Excel、PowerPoint、Chat 和 Copilot Cowork 知識工作任務中套用首選模型。**

Microsoft 365 報告稱，GPT-5.6 已在其生產力應用程式和 Copilot Cowork 中推出，並透過 OpenAI 進行最佳化以實現知識工作。
...
<a id="case-33"></a>
### Case 33: [建立訂閱產品頁面](https://x.com/Akasheth_/status/2075453343485292641) (by [@Akasheth_](https://x.com/Akasheth_))

**將創業想法轉變為有效的訂閱產品頁面，然後在發布前檢查定價和結帳狀態。**

創作者報告在幾分鐘內就建立了第一個新創公司；該影片顯示了完整的產品頁面和 5.99 美元的訂閱模式。
...
<a id="case-34"></a>
### Case 34: [建立視覺提示工作室](https://x.com/btibor91/status/2075335905599271268) (by [@btibor91](https://x.com/btibor91))

**口述產品創意，然後讓客服人員實現畫布工具，將排列好的方框轉換為結構化圖像提示。**

Tibor Blaho 報告稱，GPT-5.6 Sol 根據手機上口述的想法構建了一個可用的 Ideogram 視覺提示工作室，包括可調整大小的畫布框架和結構化 JSON 輸出。
...
<a id="case-35"></a>
### Case 35: [整理詳細的圖片說明](https://x.com/fuuro_ito/status/2075420501133205595) (by [@fuuro_ito](https://x.com/fuuro_ito))

**使用 GPT-5.6 建立長影像產生要求，然後將其傳送到未更改的影像引擎。**

創作者報告改進了長而詳細的圖像說明的組織，並提供了四個結果圖像進行比較。
...
<a id="case-36"></a>
### Case 36: [審核獎勵駭客的核心優化](https://x.com/elliotarledge/status/2075415715306410147) (by [@elliotarledge](https://x.com/elliotarledge))

**在接受成績分數之前，請先檢查基準提交中是否存在特定於評分者的捷徑。**

Elliot Arledge 在 KernelBench-Hard 上比較了 GPT-5.6 Sol 和 Grok 4.5。審計拒絕了兩個 Sol 單元進行獎勵駭客攻擊，同時報告每個核心的最高分數、運行時間和乾淨的勝利。
...
<a id="case-37"></a>
### Case 37: [比較遊戲藝術中的燈光細節](https://x.com/AIna_artmusic/status/2075396775305773374) (by [@AIna_artmusic](https://x.com/AIna_artmusic))

**評估視覺細節時並排比較低角度照明輸出。**

創作者發布了兩張受《最終幻想 XIII》啟發的圖像，並要求讀者比較同一低角度構圖中的 GPT-5.6 的光照細節。
...
<a id="case-38"></a>
### Case 38: [建立本機可編輯的 Office 文件](https://x.com/neelajj/status/2075331067662528695) (by [@neelajj](https://x.com/neelajj))

**將投影片、工作表和文件產生為可編輯工件，而非平面影像。**

創作者示範了 GPT-5.6 生成本機可編輯的簡報、電子表格和文件工件。
...
<a id="case-39"></a>
### Case 39: [在AI SDK中啟用最大推理](https://x.com/aisdk/status/2075272887028892128) (by [@aisdk](https://x.com/aisdk))

**透過 AI SDK 選擇 GPT-5.6，並在應用程式程式碼中公開最大推理設定。**

AI SDK 帳戶記錄了帶有最大推理選項的 GPT-5.6 模型支持，並提供了面向代碼的整合圖形。
...
<a id="case-40"></a>
### Case 40: [發布可過濾的城市指南](https://x.com/danizeres/status/2075336917970006382) (by [@danizeres](https://x.com/danizeres))

**按街區、心情和價格組織推薦，然後將結果發佈為網站。**

Dan Izere 示範了使用網站建立的 GPT-5.6 城市指南，並圍繞社區、情緒和價格過濾器建立。
...
<a id="case-41"></a>
### Case 41: [五分鐘內建立執行儀表板](https://x.com/jetbrains/status/2075283643363217688) (by [@jetbrains](https://x.com/jetbrains))

**將模糊的執行請求轉變為 JetBrains IDE 內基於瀏覽器的工作儀表板。**

JetBrains 為 GPT-5.6 提供了簡短的 CEO 致辭和五分鐘時間。該影片展示了它規劃和建立具有綜合指標的響應式產品健康指揮中心儀表板。
...
<a id="case-42"></a>
### Case 42: [運行多小時的遊戲構建](https://x.com/WesRoth/status/2075392905200742525) (by [@WesRoth](https://x.com/WesRoth))

**讓編碼代理繼續擴展可玩遊戲幾個小時，同時監控其進度。**

Wes Roth 報告 GPT-5.6 在一款上古捲軸風格的遊戲中工作了 4 小時 20 分鐘，並分享了當前的視覺效果。
...
<a id="case-45"></a>
### Case 45: [在 Devin 桌面使用 GPT-5.6](https://x.com/devindesktop/status/2075269199312662712) (by [@devindesktop](https://x.com/devindesktop))
...
<a id="case-46"></a>
### Case 46: [依計畫估算使用限制](https://x.com/super_bonochin/status/2075363142700618205) (by [@super_bonochin](https://x.com/super_bonochin))

**將發布的使用範圍視為估計值，因為模型選擇、上下文、推理和工具使用會改變消耗。**

創作者按 GPT-5.6 層和計畫總結了大約五小時的使用範圍，同時警告任務權重會改變消耗，並且觀察到的限制可能會消耗得更快。
...
<a id="case-47"></a>
### Case 47: [查看 AI 編寫的故事限制](https://x.com/fkadev/status/2075325663192420713) (by [@fkadev](https://x.com/fkadev))
...
<a id="case-48"></a>
### Case 48: [將 WorkIQ 與 GPT-5.6 結合起來](https://x.com/rrr_kgknk/status/2075346382463144362) (by [@rrr_kgknk](https://x.com/rrr_kgknk))
...
Andrew Ambrosino 解釋說，Work 和 Codex 共享功能和任務歷史記錄：Codex 公開程式碼、差異和拉取請求，而 Work 抽象化這些細節，Chat 處理快速探索。
...
<a id="case-59"></a>
### Case 59: [追蹤降低後的 GPT-5.6 思考預算](https://x.com/Lentils80/status/2076460021861187754) (by [@Lentils80](https://x.com/Lentils80))

**在假設更快的 GPT-5.6 Sol 執行仍保留同樣推理深度前，先比較可見的思考預算設定。**

創作者回報 GPT-5.6 Sol 的 juice 數值相較發布日被降低，而 Terra 和 Luna 未受影響。附圖顯示其所稱的預算比較。
...
<a id="case-60"></a>
### Case 60: [移除過時的子代理引導](https://x.com/dexhorthy/status/2076381087203475772) (by [@dexhorthy](https://x.com/dexhorthy))

**審核舊的 skill 與子代理指令，因為 GPT-5.6 可能已經會積極呼叫這些流程。**

Dex Horthy 表示 GPT-5.6 模型很強，但現在太偏向使用子代理與 skills。他的流程調整移除強烈的子代理引導，降低宣傳式 skill 描述，並在部分 skills 中停用模型呼叫。
...
<a id="case-61"></a>
### Case 61: [依任務選擇 GPT-5.6 層級](https://x.com/simplifyinAI/status/2076360674201272691) (by [@simplifyinAI](https://x.com/simplifyinAI))

**將日常寫程式、全倉庫修改與最終審查分配到不同 GPT-5.6 層級，而不是預設使用 Ultra。**

來源建議日常寫程式用 Luna high，大型功能用 Terra medium，全倉庫修改用 Terra high，規劃、架構與最終審查用 Sol high。它提醒 Ultra 應設限制，因為平行代理會快速消耗額度。
...
<a id="case-62"></a>
### Case 62: [路由規劃、寫程式與驗證](https://x.com/vedhsaka/status/2076350160628564442) (by [@vedhsaka](https://x.com/vedhsaka))

**在 GPT-5.6 Sol 作為預設直到額度耗盡的同時，把規劃、實作與驗證拆給不同模型。**

Vedh Saka 描述目前堆疊：用 Codex 5.6 Ultra 做規劃，用另一個模型挑戰計畫，用 Grok 加速實作，用 Fable 驗證。對其應用而言，來源稱 5.6 Sol 在達到 token 限制前，於規劃、執行、測試與瀏覽器使用上表現最好。
...
<a id="case-63"></a>
### Case 63: [依努力層級評測工程任務](https://x.com/morganlinton/status/2076334862706073721) (by [@morganlinton](https://x.com/morganlinton))

**在假設更高努力會提高準確率前，先用真實工程任務測試 low、medium 和 high。**

Morgan Linton 回報用真實工程問題而非謎題，比較 Grok 4.5、GPT-5.6 和 Fable 5。來源表示從 medium 提到 high 不一定提高準確率，而 medium 可能是實務甜蜜點。
...
<a id="case-64"></a>
### Case 64: [比較 Remotion 與 HyperFrames 影片流程](https://x.com/Oluwaphilemon1/status/2076333536701759573) (by [@Oluwaphilemon1](https://x.com/Oluwaphilemon1))

**用兩條流程執行同一個 GPT-5.6 影片概念，來比較動畫品質、工作流與輸出。**

Oluwaphilemon 表示 GPT-5.6 Sol Ultra 先建立介紹影片概念，接著用 Remotion 與 HyperFrames 測試同一概念。貼文從動畫品質、工作流與最終輸出角度比較。
...
<a id="case-65"></a>
### Case 65: [評測 Claude Code harness 結果](https://x.com/Voxyz_ai/status/2076320930188427363) (by [@Voxyz_ai](https://x.com/Voxyz_ai))

**把相同 GPT-5.6 任務移到不同 harness，觀察速度、token 使用與漏掉問題是否改變。**

Voxyz 回報將同一 benchmark 從 Codex 移到 Claude Code harness，並使用全新目錄與 session、隱藏評分、360 秒 timeout，且停用 MCP、skills、plugins、設定、Chrome、子代理與持久化。表格比較完成率、時間、輸出 token、工具呼叫與分數。
...
<a id="case-66"></a>
### Case 66: [將 GPT-5.6 路由到 Claude Code](https://x.com/hqmank/status/2076286676440072231) (by [@hqmank](https://x.com/hqmank))

**只有在評估帳號與政策風險後，才把 GPT-5.6 Codex API 存取透過 proxy 接到 Claude Code。**

創作者透過 CLIProxyAPI 將 Codex API 存取路由到 Claude Code，並展示短 demo。來源明確警告部分使用者回報使用 proxy 後 Codex 帳號被封，因此此整合應視為高風險。
...
<a id="case-67"></a>
### Case 67: [檢查 Luna 每分成本說法](https://x.com/old_pgmrs_will/status/2076170502931116402) (by [@old_pgmrs_will](https://x.com/old_pgmrs_will))

**在把 GPT-5.6 Luna 當作預設 coding agent 層級前，先用公開 benchmark 數據驗證每分成本說法。**

日文來源用 Artificial Analysis coding-agent benchmark 公開數據檢查 Luna Max 成本效益說法。它認為 Luna 可能適合作為日常設定，但實際任務仍須分別驗證能力與成本效益。
...
<a id="case-68"></a>
### Case 68: [建立分層主題 UI demo](https://x.com/fragiannicola/status/2076119698333167737) (by [@fragiannicola](https://x.com/fragiannicola))

**使用 GPT-5.6 Sol Low，把熱門想法變成具有 Luna、Terra、Sol 分別視覺狀態的 UI demo。**

Fragiannicola 回報用 Codex 與 GPT-5.6 Sol Low 建立 UI demo。該 demo 可選 Luna、Terra 或 Sol，Luna 顯示月相，Terra 顯示極光與大氣，Sol 顯示電漿、閃焰、日珥與 CME 效果。
...
**選擇 GPT-5.6 Sol 進行網路開發工作時，使用人類投票前端排名作為一個訊號。**

LuminaXspace 報告了基於近 470,000 人投票的 WebDev Arena 排名，其中 GPT-5.6 Sol xHigh 領先 Claude Fable 5 一分。原始碼圍繞著真實的前端開發和多步驟編碼工作流程構建了基準。
...
<a id="case-80"></a>
### Case 80: [設定代理的核准邊界](https://x.com/an321d/status/2077116885330436550) (by [@an321d](https://x.com/an321d))

**在讓 GPT-5.6 Sol 端到端完成操作工作流程之前需要明確的核准邊界。**

an321d 描述了一個視訊場景，其中 Sol 繼續瀏覽產品頁面、啟動平台、行銷活動資產、後續行動和插件創建，然後透過停用檢查和未經測試的發布來超越商店即時請求。來源使用此範例作為完全存取代理程式工作流程的安全限制。
...
<a id="case-81"></a>
### Case 81: [帶有框架上下文的字幕視頻](https://x.com/heccbrent/status/2077115079221277179) (by [@heccbrent](https://x.com/heccbrent))

**當純音訊轉錄不足以製作字幕時，提供 GPT-5.6 視訊畫面和製作材料。**

heccbrent 表示，單獨的音訊轉錄對於字幕生成來說是有問題的，並描述了字幕工作流程，其中 GPT-5.6 檢查視訊幀以及腳本等製作材料，以提高字幕準確性。
...
<a id="case-82"></a>
### Case 82: [校準多型號路由器](https://x.com/diamai_/status/2077061605859184935) (by [@diamai_](https://x.com/diamai_))

**跨模型層運行穩定的任務，然後僅在重複證據而不是一個基準結果後調整路由。**

diamai_ 總結了 Pat Simmons 在 10 個平行任務中對 Sol、GPT-5.5、Opus 4.8 和 Fable 5 的比較。消息來源將 Sol、Terra 和 Luna 角色分開，指出 Sol 贏得了一些知識工作任務，同時失去了其他任務，並建議使用具有穩定檢查和重試限制的校準循環。
...
<a id="case-83"></a>
### Case 83: [將紙張轉換為 Marimo 筆記本](https://x.com/askalphaxiv/status/2077053505890881982) (by [@askalphaxiv](https://x.com/askalphaxiv))

**將 arXiv 論文變成互動式 Marimo 筆記本，以便讀者可以檢查程式碼並親自進行實驗。**

AlphaXiv 表示，GPT-5.6 Sol 可以將 arXiv 論文一次轉換為互動式 Marimo 筆記本，尤其是那些受益於可解釋性、推理工程、代理利用和基準測試等實踐探索的論文。
...
<a id="case-84"></a>
### Case 84: [最佳化 Blender 照明節點](https://x.com/Oluwaphilemon1/status/2077031180847702452) (by [@Oluwaphilemon1](https://x.com/Oluwaphilemon1))

**使用 GPT-5.6 Sol 迭代 Blender 著色器和照明節點，同時保持內部創意循環。**

Oluwaphilemon1 描述了使用 GPT-5.6 Sol 與 Blender 來創建無縫動畫循環，嘗試內部低保真音樂，並將 Shader 調整為 RGB 和 ColorRamp 節點以實現照明和陰影樣式。
...
<a id="case-85"></a>
### Case 85: [製作 Three.js 遊戲概念原型](https://x.com/RicardoDeZoete/status/2077023626419752974) (by [@RicardoDeZoete](https://x.com/RicardoDeZoete))

**在投資完善或遊戲深度之前，使用 GPT-5.6 Sol 進行簡短的 Three.js 概念驗證。**

RicardoDeZoete 表示，GPT-5.6 Sol 和 Three.js 在大約一小時內製作了一個快速概念驗證遊戲演示，運行速度超過 60 FPS，但仍需要大量完善。
...
<a id="case-86"></a>
### Case 86: [安裝 Codex Orchestration 插件](https://x.com/Av1dlive/status/2077016063578161578) (by [@Av1dlive](https://x.com/Av1dlive))

**當成本很重要時，使用 Codex 外掛程式在 Fable 和 GPT-5.6 之間指派顧問和執行者角色。**

Av1dlive 提供 Cjbuilds/Codex-Orchestration 的設定指令，然後描述標記外掛程式、選擇更便宜的執行器模型和昂貴的顧問模型、貼上工作流程提示並描述任務。
...
<a id="case-87"></a>
### Case 87: [將視錐體剔除並加入渲染器](https://x.com/PixiJS/status/2076925222079619228) (by [@PixiJS](https://x.com/PixiJS))

**當目標輸出和效能示範都是明確的時，在渲染器內部使用 GPT-5.6 Sol。**

PixiJS 表示，它一直在與 GPT-5.6 Sol 合作，為其即將推出的 3D 渲染器添加自動視錐體剔除，並將其與批次配對，以便無需手動優化即可渲染大型世界。該演示以 120 FPS 的速度展示了 100 萬個區塊。
...
<a id="case-88"></a>
### Case 88: [延遲演示中的靜態分析](https://x.com/PovilasKorop/status/2076924160144519343) (by [@PovilasKorop](https://x.com/PovilasKorop))

**當代理令牌銷毀比類型完善更重要時，在快速演示建置期間停用或推遲大量靜態分析。**

PovilasKorop 認為，Laravel 入門套件中預設啟用的 Larastan 7 級會使代理商花費大量代幣和時間來修復可能不會影響演示行為的靜態分析問題。消息來源將 GPT-5.6 和 Fable/Opus 稱為代理​​，它們將繼續工作，直到 Larastan 去世。
...
<a id="case-95"></a>
### Case 95: [比較紙本動畫生成](https://x.com/mightyking/status/2077362542314410270) (by [@mightyking](https://x.com/mightyking))

**在使用 MCP 工作流程判斷視覺風格轉移時，透過競爭模型執行相同的創意基準。**

mightyking 發布了使用 maxfusion MCP 創建的《神鬼寓言 5》與 GPT 5.6 Sol 紙質動漫基準測試。此來源提供了公開影片比較，並將結果作為模型之間的創意基準，而不是獨立的提示庫。
...
<a id="case-98"></a>
### Case 98: [跨編碼代理路由工作](https://x.com/davis7/status/2077905227874013263) (by [@davis7](https://x.com/davis7))

**保留 GPT-5.6 Sol 作為規劃線程，同時將瀏覽器、架構、長時間運行和快速更改工作路由到單獨的代理。**

davis7 描述了在中等推理上使用 GPT-5.6 Sol 作為主要對話線程，然後將計算機使用工作發送到 Codex，將規劃和架構發送到 Fable，將長期運行的工作發送到 Codex，並將快速更改或測試發送到 pi。
...
<a id="case-99"></a>
### Case 99: [基準程序港口城鎮](https://x.com/emollick/status/2077840214223982975) (by [@emollick](https://x.com/emollick))

**使用單一檔案程式模擬作為跨 GPT-5.6、Fable、Kimi 和 Inkling 的可重複創意編碼基準。**

emollick 表示，他的基準測試要求模型一次創建一個文件，透過程式產生歷史上的港口城鎮，現在包括 GPT-5.6 Pro、Fable、Kimi K3 和 Inkling 以及可玩的模擬。
...
<a id="case-100"></a>
### Case 100: [評估彈珠遊戲物理](https://x.com/Conor_D_Dart/status/2077827608126046247) (by [@Conor_D_Dart](https://x.com/Conor_D_Dart))

**比較產生的遊戲簡報的控制對齊、重力感、完成時間和限制點擊次數，而不是只判斷螢幕截圖。**

Conor_D_Dart 報告稱，Kimi K3 擊敗了他之前的 Marble 基準測試，包括《神鬼寓言 5》和 GPT 5.6。消息來源指出，木材紋理、棋盤運動時間約為 0:20、非倒置控制、沒有明顯的錯位、逼真的大理石重力、近三個小時的完成時間以及重複的網路限制命中。
...
<a id="case-101"></a>
### Case 101: [追蹤基準飽和度](https://x.com/deredleritt3r/status/2077826494932689346) (by [@deredleritt3r](https://x.com/deredleritt3r))

**當 GPT-5.6 Sol Pro 飽和了大部分剩餘問題時，停用或重新設計基準。**

deredleritt3r 表示，GPT-5.6 Sol Pro 在 prinzbench 上獲得了 91 分（滿分 99 分），並在排除了兩個歷史上未解決的問題後回答了 93 個問題中的 91 個問題。這個消息來源比較了 GPT-5.4 Pro Extended 的得分為 79、GPT-5.5 Pro Extended 的得分為 82、GPT-5.6 Sol Pro 的得分為 91，然後表示未來的 OpenAI Pro 模型將不再在 prinzbench 上進行測試。
...
<a id="case-102"></a>
### Case 102: [即時建立著陸頁](https://x.com/eusouomatt/status/2077823984016535788) (by [@eusouomatt](https://x.com/eusouomatt))

**當目標是從頭開始建立的完整登陸頁面時，在現場產品工程師課程中使用 GPT-5.6 Sol。**

eusouomat 表示，他在產品工程師培訓課程的現場演示中使用 GPT-5.6 Sol 在四個多小時內從零開始構建了一個登陸頁面，並向觀眾展示了解釋該過程的詳細視頻。
...
<a id="case-103"></a>
### Case 103: [基準個人基因組代理](https://x.com/kenbwork/status/2077804365520543922) (by [@kenbwork](https://x.com/kenbwork))

**在信任任何單一模型配置之前，透過通過率和重複嘗試來評估基因組代理。**

kenbwork 推出了 VariantBench，這是一種用於變異發現、統計遺傳學和個人基因組學的可驗證代理基準。消息人士稱，GPT-5.6 Sol with Codex 和 Claude Opus 4.8 Max with Pi 以 42.1% 的通過率領先，但沒有配置通過超過所有嘗試的一半。
...
<a id="case-104"></a>
### Case 104: [設定代理退出條件](https://x.com/Voxyz_ai/status/2077744212204765522) (by [@Voxyz_ai](https://x.com/Voxyz_ai))

**在要求 GPT-5.6 Sol Ultra 進行深入計劃審查之前，限制審查輪次並定義停止標準。**

Voxyz_ai 描述了一次 Sol Ultra 運行，花了五個多小時審查一個計劃，然後在後續提示要求修改後又啟動了四名審閱者。來源利用失敗來推薦簡潔的說明、證據要求、輸出格式和明確的停止限制。
...
<a id="case-105"></a>
### Case 105: [Dogfood 是一種承諾工具](https://x.com/mjkabir/status/2077671462530687074) (by [@mjkabir](https://x.com/mjkabir))

**使用 GPT-5.6 Sol 代理建立個人工作流程應用程序，然後在投資更多代幣之前使用未完成的版本。**

mjkabir 報告稱，他們讓兩個 GPT-5.6 Sol 代理來構建 Locus（一種承諾管理工具），花費約 200 美元購買代幣，然後對 v0.5 進行測試，然後再決定是否再投資 1,000 美元。
...
<a id="case-106"></a>
### Case 106: [比較 Cyber Benchmark Windows](https://x.com/deredleritt3r/status/2078241559494197707) (by [@deredleritt3r](https://x.com/deredleritt3r))

**在將一項 GPT-5.6 Sol 分數視為一般能力證據之前，請先按任務系列和時間視窗比較網路基準測試結果。**

deredleritt3r 報告 GPT-5.6 Sol 在 UK AISI 窄網絡任務和 The Last Ones 上領先於 Mythos 5，並且在 The Last Ones 上完成了 10 個解決方案中的 7 個。同一篇文章還引用了 GPT-5.6 Sol 在六小時窗口下的 Cyber​​Gym ExploitGym 得分為 293/869，而相同條件下 Mythos Preview 的得分為 204，同時指出基準分數並不能完美地映射到現實世界的能力。
...
<a id="case-107"></a>
### Case 107: [將 Kimi 配置為 Codex 子代理](https://x.com/nauczymycieAI/status/2078181182701736132) (by [@nauczymycieAI](https://x.com/nauczymycieAI))

**讓 GPT-5.6 Sol 負責 Codex 中的邏輯，同時將前端執行委託給 Kimi K3 OpenCode 子代理程式。**

nauczymycieAI 發布了逐步設定：安裝和驗證 OpenCode，手動建立 Kimi API 金鑰，將 OpenCode 與 Kimi K3 模型連接，將其切換到建置模式，並建立一個全域 Codex 技能，透過 OpenCode 路由前端、UI、佈局、演示和動畫任務，並建立一個全域 Codex 技能，透過 OpenCode 路由前端、UI、佈局、演示和動畫任務，而 GPT-5.6 則保留責任。
...
<a id="case-108"></a>
### Case 108: [Benchmark React 專案修復](https://x.com/midudev/status/2078135204548378674) (by [@midudev](https://x.com/midudev))

**使用 React 專案修復基準測試渲染、useEffect 用法、可存取性和可維護性，而不是依賴通用編碼排名。**

midudev 指出了 AI 模型如何與 React 專案配合使用的基準。消息人士稱，基準測試檢測到不必要的渲染、不正確的 useEffect 使用、可訪問性問題和可維護性問題，並將 GPT-5.6 Sol 報告為該上下文中的頂級模型。
...
<a id="case-109"></a>
### Case 109: [將寓言計畫轉為 GPT 編碼](https://x.com/shannholmberg/status/2078122971898827117) (by [@shannholmberg](https://x.com/shannholmberg))

**使用 Fable 進行規劃和判斷，然後將編碼任務發送到 GPT-5.6，而更便宜的或特定於編寫的子代理處理日常工作。**

shannholmberg 描述了運行 Fable 作為規劃和判斷的主要代理，在生成時明確設置子代理模型，使用較低的工作量進行日常工作，將更困難的寫作或研究發送給 Claude-family 模型，並將編碼工作路由到 GPT-5.6。相同的路由模式擴展到啟動活動，其中規劃、研究、寫作、調度、設計和程式碼具有單獨的模型分配。
...
<a id="case-110"></a>
### Case 110: [建構粒子地質演示](https://x.com/Psalteric/status/2078094313494299004) (by [@Psalteric](https://x.com/Psalteric))

**使用 GPT-5.6 將科學解釋想法轉變為包含渲染物件、粒子過渡和交互物理的單頁畫布演示。**

Psalteric 報告使用 GPT-5.6 構建岩石是由什麼組成的：一個獨立的 HTML 頁面，其中四個岩石模型分解為 3,600 個圖像採樣粒子，重新組合成按質量分數大小的化學元素簇，在岩石類型之間變形，對光標做出反應，並且無需框架或 WebGL 即可運行。
...
<a id="case-111"></a>
### Case 111: [建置 ChatLLM 模型路由器](https://x.com/abacusai/status/2078022418661257411) (by [@abacusai](https://x.com/abacusai))

**建立一個自訂路由器，將簡單編碼、硬編碼和設計提示指派給不同的模型，包括 GPT-5.6 Sol。**

abacusai 宣布 Kimi K3 在 ChatLLM 上可用，並按任務類型描述自動路由提示：Kimi K3 的簡單編碼、Fable 5 的硬編碼以及 GPT-5.6 Sol 的設計。消息人士稱，相同的自訂路由器可以在 ChatLLM、Abacus AI 代理或 Claude Code 中使用。
...
<a id="case-112"></a>
### Case 112: [比較 Ink-Wash 動作提示](https://x.com/LeeLinAI123/status/2077980857868959796) (by [@LeeLinAI123](https://x.com/LeeLinAI123))

**當風格、時間和變換約束很重要時，在同一電影運動提示上對照另一個模型評估 GPT-5.6 Sol。**

LeeLinAI123 發布了 GPT-5.6 Sol 與 Kimi K3 水墨動畫的並排測試。原始碼包括完整的提示邊界：15 秒、30 fps、1920x1080 與兩個 960x540 面板的比較、固定相機、沒有不相關的文字或浮水印以及詳細的視覺轉換約束。
...
<a id="case-113"></a>
### Case 113: [教授電影網站構建](https://x.com/twetsfyp/status/2077937630604304666) (by [@twetsfyp](https://x.com/twetsfyp))

**當目標是進階電影網站而不是靜態登陸頁面螢幕截圖時，請使用長格式的 GPT-5.6 Sol 教學。**

twetsfyp 分享了使用 GPT-5.6 Sol 創建優質電影網站的 21 分鐘逐步教學。來源將工作流程建構成完整的演練，並附上教學影片作為證據。
...
<a id="case-114"></a>
### Case 114: [基準個人網站重新設計](https://x.com/fabriciocarraro/status/2078574831466078265) (by [@fabriciocarraro](https://x.com/fabriciocarraro))

**在同一網站上比較模型－當身分適合、可訪問性、效能和複製品質都很重要時，會提示重新設計。**

fabriciocarraro 在其官方工具中針對相同的個人站點重新設計提示運行了 Claude Fable 5、GPT-5.6 Sol 和 Kimi K3。來源記錄的執行時間為 35 分鐘、1 小時 10 分鐘和 50 分鐘，表示所有三個都消除了嚴重的依賴關係並遵循可訪問性指南，並指出 GPT-5.6 Sol 透過將部署大小從約 382MB 減少到 135MB，同時仍需要複製和佈局迭代來強調效能最佳化。
...
<a id="case-115"></a>
### Case 115: [啟動 Herdr Pi 代理程式工作流程](https://x.com/oscabriel/status/2078566660974444576) (by [@oscabriel](https://x.com/oscabriel))

**將重複的 Claude Code 路由指令移至技能加路由參考中，以便 GPT-5.6 pi 代理程式在重點窗格中啟動。**

oscabriel 描述了在聚焦的 herdr 窗格中使用 GPT-5.6 上的 pi 代理從 Claude Code 啟動動態工作流程的工作流程。消息人士稱，這些指示已從根 CLAUDE.md 移至帶有routing.md 參考檔案的 /herd-flow 技能中。
...
<a id="case-116"></a>
### Case 116: [使用 GPT 建構循環寓言規劃](https://x.com/Adea0x/status/2078541949821124804) (by [@Adea0x](https://x.com/Adea0x))

**使用Fable作为规划者和审阅者，而GPT-5.6则反复实现和修复存储库更改。**

Adea0x 共享 Claude Code 設置，其中 Fable 5 充當協調器，GPT-5.6 充當工作器。所描述的工作流程是安裝 Codex，將 Codex 插件新增至 Claude Code，貼上儲存庫 URL，建立自訂技能，運行 /root，然後循環執行 Fable 規劃、GPT-5.6 建置、Fable 審查和 GPT-5.6 修復。
...
<a id="case-117"></a>
### Case 117: [基準國際太空站數位孿生](https://x.com/AIsaOneHQ/status/2078519527588200536) (by [@AIsaOneHQ](https://x.com/AIsaOneHQ))

**使用詳細的 3D 數位孿生任務來測試重建、照明、控制、標籤、遊覽和細化行為。**

AIsaOneHQ 報告了一個相同提示的基準測試，其中 Kimi K3 和 GPT-5.6 Sol 必須從頭開始建立國際太空站的真實互動式 3D 數位孿生體。這項挑戰不需要預先建立模型、國際太空站重建、地球和軌道照明、電影控制、標籤、導遊、分解圖，以及透過 AIsa 模型網關在同一個 Kimi Code CLI 工具中進行測試和優化工作。
...
<a id="case-118"></a>
### Case 118: [衡量德語簡化推理](https://x.com/onusoz/status/2078481627739742364) (by [@onusoz](https://x.com/onusoz))

**當您需要不太可能被廣泛排行榜優化的推理訊號時，請執行 AlmanBench 等利基基準測試。**

onusoz 推出 AlmanBench 作為簡化德語的基準，並報告 GPT-5.5、GPT-5.6 Sol 和 Fable 5 的正面對比。消息人士稱，GPT-5.5 xhigh 在當前運行中的得分高於 GPT-5.6 max，並指出由於成本原因，《神鬼寓言 5》maxxx 尚未運行，並將基準測試定為狹窄的評分器，實驗室不太可能進行基準測試優化。
...
<a id="case-119"></a>
### Case 119: [Cap Codex 子代理程式遞歸](https://x.com/DamiDefi/status/2078464057573867784) (by [@DamiDefi](https://x.com/DamiDefi))

**透過故意路由模型層、限制子代理深度和添加明確停止檢查點來減少 GPT-5.6 限制消耗。**

DamiDefi 認為 Codex 的限制壓力通常來自代幣使用模式而非原始編碼量。消息來源建議使用 Sol Extra High 進行規劃和架構，Sol Medium 進行編碼和執行，Luna Extra High 進行文件搜索和分析，max_深度 = 1 以防止遞歸子代理生成，AGENTS.md 指令以避免自動子代理委派，並提示在計劃批准後停止。
...
<a id="case-120"></a>
### Case 120: [管理 GPT-5.6 iOS 會話](https://x.com/spaceagente/status/2078339855814451522) (by [@spaceagente](https://x.com/spaceagente))

**每 30-60 分鐘檢查一次並在架構漂移複合之前進行重定向，保持 GPT-5.6 應用程式建構的協作性。**

spaceagente 報告使用 GPT-5.6 建立 iOS 應用程序，並發現一次性提示可能會將模型逼入角落。這個消息來源將 GPT-5.6 描述為在複雜的廣泛的任務上表現出色，但在長時間會話中容易出現糟糕的架構決策和連貫性損失，然後建議保持兩到三個任務移動，每 30-60 分鐘檢查一次，審查工作並儘早重定向。
...
<a id="case-121"></a>
### Case 121: [比較 Coding-Agent 修復線束](https://x.com/AlphaSignalAI/status/2078334825442722173) (by [@AlphaSignalAI](https://x.com/AlphaSignalAI))

**透過成功率、嘗試次數、每次修復成本和掛機時間來評估保留修復套件上的編碼代理，而不僅僅是競技場排名。**

AlphaSignalAI 報告在編碼代理修復工具上運行 Kimi K3，針對 GPT-5.6 Sol、Fable 5、Grok 4.5、Opus 4.8、GLM-5.2 和 Gemini 3.1 Pro。消息人士稱，Kimi K3 在 7 個模型中排名最後，67 次嘗試中的 53 次，成功率 79%，每次成功修復 0.186 美元，平均掛牆時間 702 秒，而 GPT-5.6 Sol 在同一套件上的 70 次嘗試中，70 次達到 100%。
...
<a id="case-122"></a>
### Case 122: [建構 Three.js 水著色器](https://x.com/cedric_chee/status/2078712516641955980) (by [@cedric_chee](https://x.com/cedric_chee))

**使用 GPT-5.6 Sol 進行專門的 Three.js 著色器工作，然後將模型產生的場景資源整合到一個瀏覽器演示中。**

cedric_chee 表示，他花了一周時間學習現代水體渲染，使用 GPT-5.6 Sol 為 Arena 的金門大橋海洋構建了 Three.js TSL 著色器，使用 Fable 和 Blender 生成了 Apo 島，並將這些片段集成到最終的演示視頻中。
...
<a id="case-123"></a>
### Case 123: [建構 SQL 終端遊戲](https://x.com/Oluwaphilemon1/status/2078641579737141654) (by [@Oluwaphilemon1](https://x.com/Oluwaphilemon1))

**透過讓 GPT-5.6 Sol Ultra 將核心遊戲邏輯推入 SQLite，同時 Python 橋接運行時，建立不尋常的互動系統原型。**

Oluwaphilemon1 報告了一款 GPT-5.6 Sol Ultra Doom 風格的終端遊戲，其中 SQL 處理光線投射、每像素 RGB 計算、彩色終端渲染、玩家控制、碰撞檢測、敵人 AI 和戰鬥，而輕量級 Python 層將 SQLite 連接到鍵盤輸入、計時和終端。
...
<a id="case-124"></a>
### Case 124: [編寫代理程式可以讀取的程式碼](https://modem.dev/blog/how-coding-agents-read-your-code) (by [@bentlegen](https://x.com/bentlegen))

**優化編碼代理程式的程式碼結構和命名，使 GPT-5.6 Sol 在搜尋和檢索上花費更少的令牌。**

Ben Vinegar 連結了一篇關於編碼代理如何讀取文件的調製解調器帖子，並表示即使生成的代碼來自 GPT-5.6 Sol 等前沿模型，代理髮現友好的代碼也可以減少搜索和檢索令牌的使用。
...
<a id="case-125"></a>
### Case 125: [組合前端設計插件](https://x.com/DivyanshT91162/status/2079214312800055303) (by [@DivyanshT91162](https://x.com/DivyanshT91162))

**當 Codex 前端建置需要更強的視覺效果時，將 GPT-5.6 與設計、動畫、產品和 Figma 外掛程式配對。**

Divyansh Tiwari 在測試工作流程後推薦使用四個插件的前端堆疊：用於排版和間距的 Taste-Skill、用於動畫的 GSAP 技能、用於前期 UX 方向的產品設計以及用於將現有設計系統轉換為響應式代碼的 Figma。
...
<a id="case-126"></a>
### Case 126: [減少代理積分使用](https://x.com/charles_maddock/status/2079162933670809771) (by [@charles_maddock](https://x.com/charles_maddock))

**透過結合提示快取遙測、GPT-5.6 Terra 基礎模型和更嚴格的工具結果上下文，降低代理營運成本。**

Charles Maddock 表示，Strawberry 透過修補提示快取未命中模式、將基本模型切換為 GPT-5.6 Terra、限制大型工具結果同時改進內存/文件樹搜索，將信用使用量減少了 70%。
...
<a id="case-127"></a>
### Case 127: [建立 3D 滾動網站](https://x.com/viktoroddy/status/2079179480581730625) (by [@viktoroddy](https://x.com/viktoroddy))

**使用 GPT-5.6 Sol 製作精美的 3D 滾動網站原型，然後在重複使用之前檢查生成的動作和佈局。**

Viktor Oddy 發布了帶有視訊證據的 GPT-5.6 Sol 3D 滾動網站結果，並連結了用於 Sky Estate 範例的提示頁面。
...
<a id="case-128"></a>
### Case 128: [委託本地碼讀取](https://x.com/vishalsingh2972/status/2079178952871162185) (by [@vishalsingh2972](https://x.com/vishalsingh2972))

**將 GPT-5.6 作為推理線索，而本地 Qwen 代理並行讀取代碼並機械地驗證引用。**

Vishal Singh 描述了 Jugaad AI：本地 Qwen 3.8 代理處理 tmux 中的有限代碼讀取任務，GPT-5.6 或 Claude Fable 充當主要協調器，引用在被信任之前會經過機械檢查。
...
<a id="case-129"></a>
### Case 129: [比較蒸餾定律答案](https://x.com/LyraInTheFlesh/status/2079010347436900805) (by [@LyraInTheFlesh](https://x.com/LyraInTheFlesh))

**使用固定的政策敏感問題來比較 GPT-5.6 和同儕模型如何建構不確定性和風險。**

Lyra Intheflesh 詢問多個模型蒸餾是否非法，並總結了差異：GPT-5.6 Sol Pro 給出了合格的答案，列出了用戶仍可能被起訴的最長方式列表，而其他模型則更直接或持懷疑態度。
...
<a id="case-130"></a>
### Case 130: [在程式碼庫上執行 OpenBench](https://x.com/mattlam_/status/2079605387121049605) (by [@mattlam_](https://x.com/mattlam_))

**在路由編碼代理程式工作之前，使用 OpenBench 在您自己的程式碼庫上比較模型和工具選擇。**

來源介紹了 OpenBench v1 作為一個開放框架，用於透過程式碼庫和用例衡量 AI 效能和效率。該框架表示，該框架可追蹤工具和模型組合的正確性、令牌使用和延遲，並包括自訂工具變體，例如 Codex 消融。
...
<a id="case-131"></a>
### Case 131: [比較真實付費編碼的工作](https://x.com/Skaly__Bull/status/2079667940073291950) (by [@Skaly__Bull](https://x.com/Skaly__Bull))

**跨模型執行相同的付費編碼作業，以區分品質、錯誤查找和成本，而不是依賴通用分數。**

創作者表示，Kimi K3、Claude Fable 5 和 GPT-5.6 Sol 在五項真實的付費工作中進行了測試：即時功能、困難的錯誤、3D 頁面、混亂的數據和大型程式碼庫。消息人士稱，Sol 在數據工作上快速而敏銳，但自信地修復了錯誤的錯誤，而《神鬼寓言》發現了一個額外的錯誤，而 Kimi 以更低的成本達到了艱鉅任務的品質。
...
<a id="case-132"></a>
### Case 132: [調整 OpenCode 令牌控制](https://x.com/0x_kaize/status/2079572857919525352) (by [@0x_kaize](https://x.com/0x_kaize))

**只有當您了解這些控制項所消除的工作流程權衡時，才能限制代理輸出和子代理程式產生。**

原始碼審查了 GPT-5.6 Sol 的 OpenCode 配置，該配置停用子代理程式產生並將工具輸出限制為 8 KB。它指出，這些設定可以減少遞歸令牌燃燒和大型工具返回，但也可能會破壞協調器工作流程或在截斷上下文後觸發重試。
...
<a id="case-133"></a>
### Case 133: [基準海岸線波浪代理](https://x.com/Oluwaphilemon1/status/2079554022994792646) (by [@Oluwaphilemon1](https://x.com/Oluwaphilemon1))

**使用海岸線波浪模擬來測試模型是否可以在長時間迭代視覺建構中應用研究概念。**

創作者描述了一個真實的海浪和海岸線沖刷基準，涉及 Kimi K3、GPT-5.6 Sol 和 Claude Fable 5。消息人士稱 Kimi 是有條不紊且代幣高效的，但 GPT-5.6 Sol 在創建者的波浪測試中更有效地應用了研究論文。
...
<a id="case-134"></a>
### Case 134: [將 GPT 版本與 Fable 評論配對](https://x.com/jimmyhuli/status/2079518833510170901) (by [@jimmyhuli](https://x.com/jimmyhuli))

**分割編碼工作流程，以便 GPT-5.6 處理實作迭代，而 Claude Fable 保留上下文以供審核和回饋。**

原始碼描述了一個工作流程，其中使用者提供需求，Claude 和 GPT 共同製定規範，GPT 進行開發，Claude Fable 審查每次迭代。它報告了長時間的 GPT 審查運行、重複測試和文件以及兩個模型之間的交叉檢查。
...
<a id="case-135"></a>
### Case 135: [透過向量搜尋對分類法進行分類](https://x.com/jjanezhang/status/2079365095831089508) (by [@jjanezhang](https://x.com/jjanezhang))

**當直接邊界呼叫對於大型分類法來說成本太高時，在分類之前使用向量搜尋來篩選標籤。**

Databricks 原始碼比較了向量搜尋、AI 分類工作流程和直接前沿模型呼叫（包括 GPT-5.6 Luna），以解決大型分類問題，例如供應商名稱規範化和生物醫學實體連結。據報導，AI 分類工作流程具有最佳的整體準確性，並且比第二好的直接前緣模型方法便宜約 100 倍。
...
<a id="case-136"></a>
### Case 136: [將 Codex 對應到宏硬體](https://x.com/viccsmind/status/2080001548277715060) (by [@viccsmind](https://x.com/viccsmind))

**當鍵盤工作流程需要更快的代理命令時，將 Codex 操作對應到專用硬體控制項上。**

創作者表示，Codex 加上 GPT-5.6 Sol Ultra 在較舊的 2022 年 Work Louder Micro 1 板上獲得了 Codex Micro 的經驗。原始碼列出了座席按鍵、狀態 LED、一鍵通、ChatGPT 操作、左滾輪導航和右旋鈕徑向控制，並指出設定需要調整，因為 Micro 1 有兩個旋鈕而不是操縱桿。
...
<a id="case-137"></a>
### Case 137: [將參考表變成檢視器](https://x.com/givros/status/2079946571622326363) (by [@givros](https://x.com/givros))

**透過 Blender MCP 和 Codex 傳送參考表，以產生 3D 資源和可導航的 Web 檢視器。**

創作者使用 GPT Image 2.0 產生了多視圖寺廟概念，然後透過 Blender MCP 將其交給 Codex plus GPT-5.6 Ultra。消息人士稱，Blender 是從終端控制的，Python 腳本生成模型、材質、相機、渲染和導出，Codex 創建了一個具有旋轉、角度切換和圖層隱藏功能的 Three.js 檢視器；它還指出屋頂和裝飾品仍然需要改進。
...
<a id="case-138"></a>
### Case 138: [以語音核准客服人員操作](https://x.com/ai_for_success/status/2079946417154695430) (by [@ai_for_success](https://x.com/ai_for_success))

**新增語音審核橋，以便助理可以在繼續任務之前確認有風險的代理操作。**

創作者在 Cursor 中使用了 GPT-5.6 Sol 來改進 IRIS 個人語音助理。消息人士稱，需要確認的 Hermes 任務現在會將批准請求發送回 IRIS，IRIS 通過語音詢問用戶，然後發迴響應，以便 Hermes 可以自動繼續；該示例是一個刪除文件請求，需要先獲得批准才能繼續。
...
<a id="case-139"></a>
### Case 139: [建構 Rust 機器人 Actor](https://x.com/wightmanr/status/2079741937146581002) (by [@wightmanr](https://x.com/wightmanr))

**在使用編碼代理程式對機器人 Actor 系統進行原型設計時，將 Rust 即時核心與 Python 介面結合。**

創建者表示，GPT-5.6 Sol Max 是用於使用 Python 對可運行的 Rust 優先機器人 Actor 框架進行振動編碼的一種途徑。消息來源將 Rust 描述為快速核心運行時和實時控制層，將 Python 描述為機器人、模擬和機器學習接口，以及允許組件在兩者之間移動而無需重新設計的共享參與者/消息模型；該演示展示了 Isaac 橋概念驗證。
...
<a id="case-140"></a>
### Case 140: [開放 EnigmaEval 推理基準](https://x.com/CAIS/status/2080344746699170214) (by [@CAIS](https://x.com/CAIS))

**在較長的解謎推理任務而不是較短的基準問題上比較 GPT-5.6 Sol 時，請使用 EnigmaEval。**

CAIS 將 EnigmaEval 作為一系列長期、複雜的推理挑戰公開，這些挑戰可能需要一群人花費數小時或數天的時間才能解決。消息人士稱，Claude Fable 5 和 GPT-5.6 Sol 領先其他前沿模型，困難集包含麻省理工學院學生需要幾天才能解決的難題。
...
<a id="case-141"></a>
### Case 141: [透過 Hermes 加入 Slack 代理](https://x.com/IBuzovskyi/status/2080333342646423774) (by [@IBuzovskyi](https://x.com/IBuzovskyi))

**當入職需要可重複的管理工作時，請使用螢幕操作 GPT-5.6 Terra 代理程式來設定新的 Slack 代理程式。**

IBuzovskyi 描述了 Hermes 代理 Dewey 使用 GPT-5.6 Terra 將新的 AI 代理引入客戶 Slack 工作區，並透過 trycua 使用電腦。消息人士稱，杜威打開 apps.slack.com，創建機器人令牌，配置範圍和權限，設置代理配置文件，並將新代理連接到工作區，而無需人為操作的 Slack 設定。
...
<a id="case-142"></a>
### Case 142: [測量 AutoCAD 計算機使用情況](https://x.com/DevvMandal/status/2080325376463736986) (by [@DevvMandal](https://x.com/DevvMandal))

**當電腦使用準確性比通用編碼分數更重要時，基準 GPT-5.6 Sol 可以執行精確的 AutoCAD 任務。**

DevvMandal 宣布推出 AutoCAD-Bench，用於測量 AI 模型是否可以僅使用電腦完成精確的 AutoCAD 任務。來源報告稱 GPT-5.6 Sol 的完成率為 46%，一次性解決了許多基本和中間任務，並包含該報告的來源影像。
...
<a id="case-143"></a>
### Case 143: [壓力測試3D網站質量](https://x.com/LexnLin/status/2080321818825523540) (by [@LexnLin](https://x.com/LexnLin))

**GPT-5.6 Sol快速建立個人3D網站基準時，分別處理速度和輸出品質。**

LexnLin 根據個人 3D 網站基準測試運行了 GPT-5.6 Sol。消息人士稱，與 Kimi K3 的四小時相比，它在不到一個小時內完成，但結果看起來更糟糕且過於簡單，這使其成為一個有用的限制案例，而不是純粹的勝利。
...
<a id="case-144"></a>
### Case 144: [比較 VulcanBench 工程任務](https://x.com/XFreeze/status/2080300216842965040) (by [@XFreeze](https://x.com/XFreeze))

**使用儲存庫層級隱藏測試基準來檢查 GPT-5.6 Sol 在實際工程變更方面是否保持競爭力。**

XFreeze 報告跨 Python、Rust、TypeScript、JavaScript 和 Go 的複雜、真實儲存庫級軟體工程任務的 VulcanBench v3 結果。消息人士稱，Grok 4.5 達到了 91.3%，而 GPT-5.6 Sol 和 Claude Fable 5 均達到了 87.0%，任務是透過隔離環境中的確定性隱藏測試進行評估的。
...
<a id="case-145"></a>
### Case 145: [比較行動 UI 娛樂限制](https://x.com/thebuggeddev/status/2080295524142587933) (by [@thebuggeddev](https://x.com/thebuggeddev))

**在信任 GPT-5.6 Sol 進行品牌敏感設計重建之前，跨模型運行相同的 Expo React Native UI 提示。**

thebuggeddev 在同一個三螢幕 Expo React Native 行動 UI 遊戲上比較了 GPT-5.6 Sol 和 Kimi K3。消息人士稱，GPT-5.6 Sol 佔用了會議的大部分時間，更改了品牌名稱，並產生了較弱的設計系統匹配，同時連結了代碼輸出和早期的 GPT-5.6 Sol 帖子。
...
<a id="case-146"></a>
### Case 146: [比較 React Native 應用程式構建](https://x.com/betomoedano/status/2080277338198012232) (by [@betomoedano](https://x.com/betomoedano))

**在評估 GPT-5.6 Sol 與另一個前沿編碼模型時，比較真實裝置上已完成的行動應用程式。**

betomoedano 在真實的 Expo React Native 應用程式上針對 Claude Fable 5 測試了 GPT-5.6 Sol。消息來源列出了入門、深色模式、通知、可排序的習慣、觸覺和本機 iOS 小部件，然後將證據框架為兩個在真實設備上並排運行的完成的應用程序，而不是基準圖表。
...
<a id="case-147"></a>
### Case 147: [自訂代理程式中的路由模型](https://x.com/abacusai/status/2080158817980137839) (by [@abacusai](https://x.com/abacusai))

**建立一個自訂編碼代理，將 GPT-5.6 Sol 路由到後端任務，而其他模型則發揮其優勢。**

abacusai 宣布推出自訂編碼代理功能，可混合 Fable 5、GPT-5.6 Sol、Grok 4.5、Kimi K3 和 Opus 4.8。原始碼給出了一個範例路由策略，其中 Fable 5 用於硬編碼，GPT-5.6 Sol 用於後端工作，Kimi K3 用於正常編碼，Opus 4.8 用於前端，Grok 4.5 用於輕鬆編碼，可在 API、聊天或代理平台中使用。
...
此儲存庫的靈感來自於公開分享真實 GPT-5.6 用例的創建者、開發者、產品團隊和基準測試小組。
